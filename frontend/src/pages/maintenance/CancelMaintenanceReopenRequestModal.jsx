...
const getErrorMessage = error =>
    ...
    error?.message ||
    "Unable to cancel maintenance reopening request.";

function CancelMaintenanceReopenRequestModal({
    open,
    maintenanceRequest,
    accessContext,
    onClose,
    ...
    const loadPendingReopen = useCallback(
        async () => {
            if (
                !open ||
    ...
    const submit = async event => {
        ...
        if (!pendingReopen) {
            setError(
                "Load the pending reopening request before cancellation."
            );
            return;
        }
        ...
        if (
            ...
            setError(
                "Cancellation decision note must contain between 5 and 2000 characters."
            );
            return;
        }
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/60 p-3 sm:p-6">
            ...
            >
                <div className="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 px-5 py-4 sm:px-6">
                    ...
                    <button
                        type="button"
                        aria-label="Close cancel reopening request modal"
                        disabled={submitting}
                        onClick={onClose}
                    ...
                    <div className="space-y-5">
                        ...
                        <label className="block">
                            ...
                            <textarea
                                ...
                                maxLength={2000}
                                rows={6}
                                placeholder="Explain why this pending reopening request should be cancelled..."
                                onChange={
                                    event => {
                                        setDecisionNote(
                                            event.target.value
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs leading-5 text-slate-600">
                            Cancelling the reopening request leaves the maintenance request in its existing terminal status.
                        </div>
                    </div>
                </div>
...
export default CancelMaintenanceReopenRequestModal;
