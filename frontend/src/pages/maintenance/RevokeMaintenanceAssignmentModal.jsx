...
const getErrorMessage = error =>
    ...
    error?.message ||
    "Unable to revoke maintenance assignment.";

function RevokeMaintenanceAssignmentModal({
    open,
    ...
    const submit =
        async event => {
            ...
            if (!authoritativeAssignment) {
                setError(
                    "Refresh the assignment before revoking."
                );
                return;
            }
            ...
            if (
                ...
                setError(
                    `Assignment cannot be revoked from ${formatLabel(
                        authoritativeAssignment
                            .status
                    )}.`
                );
            ...
            if (
                ...
                setError(
                    "Assignment revocation reason must contain between 3 and 2000 characters."
                );
                return;
            }
                ...
                <div className="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 px-5 py-4 sm:px-6">
                    <div>
                        <h3 className="text-xl font-bold text-slate-950">
                            Revoke Maintenance Assignment
                        </h3>

                        <p className="mt-1 text-sm text-slate-500">
                            Owner/admin assignment control
                        </p>
                    </div>

                    <button
                        type="button"
                        aria-label="Close revoke assignment modal"
                        disabled={
                            submitting ||
                            loadingAssignment
                        }
                    ...
                    <div className="space-y-5">
                        ...
                        <label className="block">
                            ...
                            <textarea
                                ...
                                maxLength={2000}
                                rows={6}
                                placeholder="Explain why this assignment is being revoked..."
                                onChange={
                                    event => {
                                        setRevocationReason(
                                            event.target.value
...
export default RevokeMaintenanceAssignmentModal;
