...
const getErrorMessage = error =>
    ...
    error?.message ||
    "Unable to cancel the maintenance cost.";
...
function CancelMaintenanceCostModal({
    open,
    maintenanceRequest,
    maintenanceCost,
    accessContext,
    ...
    const submit = async event => {
        ...
        if (
            ...
            setError(
                "Cancellation decision note must contain between 3 and 2000 characters."
            );
            return;
        }
        ...
        try {
            ...
            if (!authoritativeCost) {
                throw new Error(
                    "The maintenance cost could not be refreshed before cancellation."
                );
            }

            if (
                authoritativeCost.status !== "submitted"
            ) {
                setError(
                    "This cancellation action is only available for a submitted cost awaiting its initial approval decision."
                );
                return;
            }
            ...
            if (
                ...
                setError(
                    "No pending initial approval exists for this maintenance cost."
                );
                return;
            }
                ...
                <div className="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 px-5 py-4 sm:px-6">
                    <div>
                        <h3 className="text-xl font-bold text-slate-950">
                            Cancel Maintenance Cost
                        </h3>

                        <p className="mt-1 text-sm text-slate-500">
                            Cancel the pending initial approval and close this submitted cost.
                        </p>
                    </div>

                    <button
                        type="button"
                        aria-label="Close cancel maintenance cost modal"
                        disabled={submitting}
                        onClick={onClose}
                    ...
                    <div className="space-y-5">
                        ...
                        <label className="block">
                            ...
                            <textarea
                                ...
                                maxLength={2000}
                                rows={5}
                                placeholder="Explain why this submitted maintenance cost is being cancelled..."
                                onChange={event => {
                                    setDecisionNote(
                                        event.target.value
                                    );
                        ...
                        <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900">
                            A cancelled maintenance cost is terminal and remains in the audit history. Its estimated amount is excluded from active request cost totals.
                        </div>
                    </div>
                </div>
...
export default CancelMaintenanceCostModal;
