 ...
 *
 * Restores a blocked tenant profile to active without
 * changing the current owner-tenant relationship.
 */
const unblockTenant = async ({
    ...
    try {
         ...
         * Tenant profile status is global across owners.
         *
         * Regular users may therefore mutate this lifecycle
         * only through the CURRENT PRIMARY owner relationship.
         * Admin may act through any current ACTIVE relationship.
         */
        let primaryRelationshipCondition = "";
        ...
        /*
         * Tenant must be current/non-deleted and the selected
         * owner relationship must still be active.
         */
        const tenantResult = await client.query(
            `
            SELECT
        ...
        /*
         * Idempotent handling for active tenant.
         */
        if (tenant.status === "active") {
            await client.query("ROLLBACK");
         ...
         * blocked -> active
         *
         * prospective/inactive use Activate Tenant instead.
         */
        if (tenant.status !== "blocked") {
            await client.query("ROLLBACK");
 ...
 *
 * Blocks the selected owner's current relationship with the
 * tenant without blocking the tenant profile globally.
 */
const blockOwnerTenantRelationship = async ({
    ownerPublicId,
    tenantPublicId,
    authenticatedUser
}) => {
    ...
    try {
        ...
        /*
         * Regular users need current property-management
        ...
        /*
         * Load and lock the selected owner's CURRENT
         * relationship with the tenant.
         *
         * Both active and blocked are current relationship
         * states because ended_at remains NULL.
         */
        const relationshipResult =
            await client.query(
                `
         ...
         * active -> blocked
         *
         * A blocked relationship cannot remain primary under
         * the database lifecycle constraint, therefore the
         * primary flag is cleared in the same atomic update.
         *
         * Tenant profile status is intentionally untouched.
         */
        const blockedResult =
            await client.query(
                `
        ...
        /*
         * Existing deferred lease integrity requires the
         * owner-tenant relationship to stay active while a
         * draft, scheduled or active lease depends on it.
         *
         * Force those checks now so the whole transaction
         * rolls back cleanly when such a lease exists.
         */
        await client.query(
            "SET CONSTRAINTS ALL IMMEDIATE"
        );
 ...
 *
 * Unblocks the selected owner's current relationship with the
 * tenant without changing the tenant profile lifecycle.
 */
const unblockOwnerTenantRelationship = async ({
    ownerPublicId,
    tenantPublicId,
    authenticatedUser
}) => {
    ...
    try {
         ...
         *
         * Important:
         * - Tenant profile status is NOT changed.
         * - Primary flag is NOT automatically restored.
         *   Blocking intentionally cleared it, and unblocking
         *   should not silently override another owner's
         *   possible primary relationship.
         */
        const unblockedResult =
            await client.query(
                `
                UPDATE owner_tenants
        ...
        /*
         * Force deferred database integrity checks before
         * committing.
         */
        await client.query(
            "SET CONSTRAINTS ALL IMMEDIATE"
        ...
        return {
            ...
            owner_relationship:
                unblockedResult.rows[0],

            unblocking_summary: {
                relationship_unblocked: true,
