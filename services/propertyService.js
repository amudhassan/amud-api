...
/**
 * Retrieve soft-deleted properties.
 *
 * Administrative recovery endpoint:
 * - Admin only.
 * - Normal property listing remains unchanged.
 * - Supports the same filters/pagination as GET /api/properties.
 * - Includes deleted_at so the recovery UI can show when deletion happened.
 */
const getDeletedProperties = async ({
    filters,
    authenticatedUser
}) => {
    /*
     * Defense in depth: deleted-property recovery
     * visibility is administrative only.
     */
    if (authenticatedUser.role !== "admin") {
        return {
            forbidden: true
